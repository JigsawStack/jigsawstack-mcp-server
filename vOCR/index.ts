...
const VOCR: Tool = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      base64Image: { type: "string", description: "A Base64 encoded string of the image." },
      prompts: {
        type: "array",
        items: { type: "string" },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  switch (request.params.name) {
    case "VOCR": {
      try {
        const { base64Image } = request.params.arguments as { base64Image: string };
        const { prompts } = request.params.arguments as { prompts: string[] };
        
        if (!isValidBase64(base64Image)) {
          throw new McpError(
            ErrorCode.InvalidParams,
            "Invalid base64 image string provided."
          );
        }
        ...
        console.error("Error processing VOCR request:", error);
        const content= [{
          type: "text",
          text: error.message
        }];
